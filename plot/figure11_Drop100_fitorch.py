...
for f in files:
    for batch in ['18650F', '21700F', '26650F']:
        if batch in f:
            try:
...
for f in files:
    for i, batch in enumerate(['18650F', '21700F', '26650F']):
        if batch in f:
            try:
                path = os.path.join(root, f)
                data = pd.read_csv(path)
...
ax.set_title('Fitorch Drop Rate (100) vs Temperature', fontsize=8)
ax.set_xlabel(r'Temperature ($^\circ$C)', fontsize=5)
ax.set_ylabel(r'Drop Rate (%)', fontsize=5)
