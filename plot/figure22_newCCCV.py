...
ax1.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, p: format(x, ',')))
ax2.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, p: format(x, ',')))
...
# 为内嵌图也添加千位分隔符（如果需要）
axins1.xaxis.set_major_formatter(ticker.FuncFormatter(lambda x, p: format(int(x), ',')))
