    ...
    for f in files:
        if batch in f:
            try:
                ...
                IR = data['Drop'].values
                all_values.update(IR)  # Collect all IR values
...
ax.set_title('Fitorch Drop Percentage vs Temperature', fontsize=8)
ax.set_xlabel(r'Temperature ($^\circ$C)', fontsize=5)
ax.set_ylabel(r'Drop Percentage (%)', fontsize=5)

ax.legend(custom_lines, legends, loc='upper right', bbox_to_anchor=(1, 1), frameon=False, ncol=1, fontsize=6)
